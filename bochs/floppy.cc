/////////////////////////////////////////////////////////////////////////
// $Id: floppy.cc,v 1.1 2006/11/27 11:18:35 jcomeau_ictx Exp $
/////////////////////////////////////////////////////////////////////////
//
...
/* for main status register (s.main_status_reg) */
#define FD_MS_MRQ  0x80
#define FD_MS_DIO  0x40
#define FD_MS_DATA_READY FD_MS_MRQ|FD_MS_DIO
#define FD_MS_NDMA 0x20
/* note: when NDMA is set, controller interrupts for every byte available
/* at data register */
#define FD_MS_BUSY 0x10
#define FD_MS_STATUSBITS FD_MS_MRQ|FD_MS_DIO|FD_MS_NDMA|FD_MS_BUSY
#define FD_MS_ACTD 0x08
#define FD_MS_ACTC 0x04
#define FD_MS_ACTB 0x02
#define FD_MS_ACTA 0x01
#define FD_MS_ACTIVEBITS FD_MS_ACTD|FD_MS_ACTC|FD_MS_ACTB|FD_MS_ACTA

#define FD_RESET 0  // useful for most registers in reset state

// define bits of status register ST0
#define FD_ST0_IC1 0x80  // interrupt code bit 1
#define FD_ST0_IC0 0x40  // interrupt code bit 0
#define FD_ST0_NORMAL_TERMINATION 0  // command terminated without errors
#define FD_ST0_ABNORMAL_TERMINATION FD_ST0_IC0  // could not finish processing
#define FD_ST0_INVALID_COMMAND FD_ST0_IC1  // could not start processing
#define FD_ST0_POLLING_FAILED FD_ST0_IC1|FD_ST0_IC0  // drive not ready
...
void bx_floppy_ctrl_c::init(void)
{
  ...
  BX_DEBUG(("Init $Id: floppy.cc,v 1.1 2006/11/27 11:18:35 jcomeau_ictx Exp $"));
  DEV_dma_register_8bit_channel(2, dma_read, dma_write, "Floppy Drive");
  DEV_register_irq(6, "Floppy Drive");
  // here is where we register ourself to handle reads and writes to these ports
...
void bx_floppy_ctrl_c::reset(unsigned type)
{
  ...
  BX_FD_THIS s.main_status_reg = FD_RESET;
  // status regs 0-3 are accessed through the data register via command 0x04
  BX_FD_THIS s.status_reg0 = FD_RESET;
  BX_FD_THIS s.status_reg1 = FD_RESET;
  BX_FD_THIS s.status_reg2 = FD_RESET;
  BX_FD_THIS s.status_reg3 = FD_RESET;
...
Bit32u bx_floppy_ctrl_c::read(Bit32u address, unsigned io_len)
{
  ...
  switch (address) {
#if BX_DMA_FLOPPY_IO
    case 0x3F2: // diskette controller digital output register (DOR)
    ...
    default:
      BX_ERROR(("read(): unsupported address 0x%04x", (unsigned) address));
      return(0);
      break;
  }
...
void bx_floppy_ctrl_c::write(Bit32u address, Bit32u value, unsigned io_len)
{
  ...
  BX_DEBUG(("write(): to port %04x, value=%02x", address, value));
  ...
  switch (address) {
    ...
    case 0x3F2: /* diskette controller digital output register */
      ...
      if (prev_normal_operation==0 && normal_operation) {
        ...
        BX_DEBUG(("write(): timer activated for floppy transition to normal"));
    ...
    case 0x3f4: /* diskette controller data rate select register */
      ...
      if (value & 0x80) {
        ...
        BX_FD_THIS s.pending_command = 0xfe; // RESET pending
        bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index, 250, 0);
        BX_DEBUG(("write(): timer activated for floppy reset"));
      }
      if ((value & 0x7c) > 0) {
    ...
    case 0x3F5: /* diskette controller data */
      ...
      if (BX_FD_THIS s.command_complete) {
        if (BX_FD_THIS s.pending_command != 0)
          BX_PANIC(("write(): port 0x03f5 receiving new command 0x%02x, old one (0x%02x) pending",
        ...
        reset_status(); // clear drive status
        ...
        switch (value) {
          ...
          case 0x18: // National Semiconductor version command; return 80h
            ...
            BX_FD_THIS s.status_reg0 = FD_ST0_INVALID_COMMAND;
            enter_result_phase();
            break;
...
{
  BX_DEBUG(("floppy_command() starting"));
  ...
  switch (BX_FD_THIS s.pending_command) {
    case 0x03: // specify
      ...
      if (BX_FD_THIS s.command[2] & 0x01) BX_FD_THIS s.main_status_reg |= FD_MS_NDMA;
      if (non_dma()) {
        BX_DEBUG(("floppy_command(): controller in non DMA mode"));
        /* other bugs must be fixed before we can unregister */
        //DEV_dma_unregister_channel(FLOPPY_DMA_CHAN);
      }
      enter_idle_phase();
      return;
    ...
    case 0x07: // recalibrate
      ...
      BX_DEBUG(("floppy_command(): timer activated for floppy recalibrate"));
    ...
    case 0x0f: /* seek */
      ...
      bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index, step_delay, 0);
      BX_DEBUG(("floppy_command(): timer activated for floppy seek"));
    ...
    case 0x4a: // read ID
      ...
      BX_DEBUG(("floppy_command(): timer activated for floppy read ID"));
    ...
    case 0x4d: // format track
      ...
      if (non_dma()) {
        BX_DEBUG(("floppy_command(): implement non-DMA floppy format"));;
      } else {
    ...
    case 0xc5: // write normal data, MT=1
      ...
      if ((BX_FD_THIS s.pending_command & 0x4f) == 0x46) { // read
        ...
        bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index,
                                    sector_time , 0);
        BX_DEBUG(("floppy_command(): timer activated for floppy read"));
      } else if ((BX_FD_THIS s.command[0] & 0x7f) == 0x45) { // write
        BX_FD_THIS s.main_status_reg |= FD_MS_BUSY;
        if (non_dma()) {
          BX_DEBUG(("floppy_command(): implement non-DMA floppy write"));;
        } else {
          /* data reg not ready, controller busy */
          DEV_dma_set_drq(FLOPPY_DMA_CHAN, 1);
        }
  ...
  switch (BX_FD_THIS s.pending_command) {
    ...
    case 0xc5:
      if (BX_FD_THIS s.TC) { // Terminal Count line, done
        ...
        if (non_dma()) {
          BX_DEBUG(("timer(): implement non-DMA floppy write"));;
        } else {
          DEV_dma_set_drq(FLOPPY_DMA_CHAN, 1);
        }
      }
    ...
    case 0xe6:
      // transfer next sector
      if (non_dma()) {
        BX_DEBUG(("timer(): implement non-DMA floppy read"));;
        BX_FD_THIS s.main_status_reg &= ~FD_MS_BUSY;
    ...
    case 0x4d: /* format track */
      if ((BX_FD_THIS s.format_count == 0) || BX_FD_THIS s.TC) {
        ...
        if (non_dma()) {
          BX_DEBUG(("timer(): implement non-DMA floppy format"));;
        } else {
          // transfer next sector
          DEV_dma_set_drq(FLOPPY_DMA_CHAN, 1);
        }
...
void bx_floppy_ctrl_c::dma_write(Bit8u *data_byte)
{
  ...
  drive = BX_FD_THIS s.DOR & 0x03;
  /*
  BX_DEBUG(("dma_write() starting, D %d, S %d, H %d B %d",
    drive, BX_FD_THIS s.sector[drive],
    BX_FD_THIS s.head[drive], BX_FD_THIS s.floppy_buffer_index));
  */
  *data_byte = BX_FD_THIS s.floppy_buffer[BX_FD_THIS s.floppy_buffer_index++];
  ...
  if ((BX_FD_THIS s.floppy_buffer_index >= 512) || (BX_FD_THIS s.TC)) {
    ...
    if (BX_FD_THIS s.TC) { // Terminal Count line, done
      BX_DEBUG(("dma_write(): end of data transfer"));
      BX_FD_THIS s.status_reg0 = (BX_FD_THIS s.head[drive] << 2) | drive;
      BX_FD_THIS s.status_reg1 = 0;
      BX_FD_THIS s.status_reg2 = 0;
      ...
      // do not assume that all floppies have two sides
      ...
      bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index,
                                  sector_time , 0);
      BX_DEBUG(("dma_write(): timer activated for floppy read"));
    }
  }
}
  ...
  if (BX_FD_THIS s.pending_command == 0x4d) { // format track in progress
    ...
    switch (3 - (BX_FD_THIS s.format_count & 0x03)) {
      ...
      case 3:
        ...
        bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index,
                                    sector_time , 0);
        BX_DEBUG(("dma_read(): timer activated for sector format"));
        break;
    }
    ...
    if ((BX_FD_THIS s.floppy_buffer_index >= 512) || (BX_FD_THIS s.TC)) {
      ...
      bx_pc_system.activate_timer(BX_FD_THIS s.floppy_timer_index,
                                  sector_time , 0);
      BX_DEBUG(("dma_read(): timer activated for sector write"));
    }
  }
}
...
{
  BX_DEBUG(("enter_result_phase() starting"));
  Bit8u drive;
  unsigned i;
  ...
  switch (BX_FD_THIS s.pending_command) {
    ...
    case 0x0e: // dump registers
      ...
      BX_FD_THIS s.result[5] = (BX_FD_THIS s.HLT << 1) | non_dma();
...
{
  BX_DEBUG(("enter_idle_phase() starting"));
  reset_status();
  BX_FD_THIS s.main_status_reg |= FD_MS_MRQ; // ready to receive
...
bx_bool bx_floppy_ctrl_c::non_dma(void)
{
  return ((BX_FD_THIS s.main_status_reg & FD_MS_NDMA) > 0);
}

void bx_floppy_ctrl_c::reset_status(void)
{
  int not_using_dma = BX_FD_THIS s.main_status_reg & FD_MS_NDMA;
  BX_FD_THIS s.main_status_reg &= FD_MS_ACTIVEBITS | not_using_dma;
}

void bx_floppy_ctrl_c::reset_all_status(void)
{
  int not_using_dma = BX_FD_THIS s.main_status_reg & FD_MS_NDMA;
  BX_FD_THIS s.main_status_reg = not_using_dma;
}
...
bx_bool bx_floppy_ctrl_c::get_tc(void)
{
  ...
  if (non_dma()) {
    ...
    // comment out the verbose debugging when it works
    /*
    BX_DEBUG(("get_tc(): eot=%d, sector=%d, byte=%d head=%d, heads=%d",
     BX_FD_THIS s.eot[drive], BX_FD_THIS s.sector[drive],
     BX_FD_THIS s.floppy_buffer_index, BX_FD_THIS s.head[drive],
     BX_FD_THIS s.media[drive].heads));
    */
  ...
  //if (terminal_count) BX_DEBUG(("get_tc(): terminal count reached"));
